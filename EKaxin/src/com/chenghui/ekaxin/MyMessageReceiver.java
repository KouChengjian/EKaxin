...
public class MyMessageReceiver extends BroadcastReceiver {
	...
	private void parseMessage(final Context context, String json) {
		...
		try {
			...
			Toast.makeText(context, " 删除函数1  ",Toast.LENGTH_LONG).show();
			// BmobConstant.PUSH_KEY_TAG 标示该消息是否属于标签消息
			...
			if(tag.equals(BmobConfig.TAG_OFFLINE)){//下线通知
				Toast.makeText(context, " 删除函数2  ",Toast.LENGTH_LONG).show();
				if(currentUser!=null){
				...
				Toast.makeText(context, " 删除函数 3 ",Toast.LENGTH_LONG).show();
				// BmobConstant.PUSH_KEY_TARGETID 标示该消息来源用户objectid   ---来源
				String fromId = BmobJsonUtil.getString(jo, BmobConstant.PUSH_KEY_TARGETID);
			    //增加消息接收方的ObjectId--目的是解决多账户登陆同一设备时，无法接收到非当前登陆用户的消息。   聊天消息的相关json字段:标示该消息接收方  --目标
				...
				if(fromId!=null && !BmobDB.create(context,toId).isBlackUser(fromId)){//该消息发送方不为黑名单用户
					if(TextUtils.isEmpty(tag)){//不携带tag标签--此可接收陌生人的消息 接收所有的聊天信息
						//Toast.makeText(context, " 陌生人  ",Toast.LENGTH_LONG).show();
						String string = jo.getString("username");
						Toast.makeText(context, string,Toast.LENGTH_LONG).show();
						if(!TextUtils.isEmpty(string)){
							Toast.makeText(context, " 删除函数 5 ",Toast.LENGTH_LONG).show();
						}else{
						...
						if(tag.equals(BmobConfig.TAG_ADD_CONTACT)){
							Log.e("TAG_ADD_CONTACT"," 添加联系人  ");
							//Toast.makeText(context, " 添加联系人  ",Toast.LENGTH_LONG).show();
							...
							Log.e("TAG_ADD_AGREE"," 同意添加联系人  ");
							//Toast.makeText(context, " 同意添加联系人  ",Toast.LENGTH_LONG).show();
							...
							Log.e("TAG_READED"," 信息已读  ");
							//Toast.makeText(context, " 信息已读  ",Toast.LENGTH_LONG).show();
