...
/**
 * 一键交换菜单
 * @ClassName: RadialMenuWidget
 * @Description: TODO
 * @author smile
 * @date 2014-9-23 下午4:18:45
 */
public class RadialMenuWidget extends View {
	...
	// 菜单的颜色   深蓝色
	private int defaultColor = Color.parseColor("#0056ff"); 
	private int defaultAlpha = 180; 
	// 好友 与弹出三子菜单背景颜色
	private int wedge2Color = Color.parseColor("#0056ff") ; 
	private int wedge2Alpha = 210;
	// 边框线 画板    背景一样的颜色
	private int outlineColor = Color.parseColor("#21252b") ; 
	private int outlineAlpha = 255; 
	// 中间菜单颜色(按下的颜色)
	private int selectedColor = Color.parseColor("#0056ff") ; 
	private int selectedAlpha = 210; 
    // 未选择的颜色  (标题菜单的颜色)
	private int disabledColor = Color.parseColor("#363c45"); 
	...
	private int headerTextColor = Color.parseColor("#ff0000"); 
	...
	public RadialMenuWidget(Context context) {
		...
		this.yPosition = (int) ((getResources().getDisplayMetrics().heightPixels) / 2.5);
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (state == MotionEvent.ACTION_DOWN) {
			...
			if (inCircle == true) {
				...
				selected = null;
				Toast.makeText(getContext(), centerCircle.getName() + " 1",
				...
				for (int i = 0; i < Wedges.length; i++) {
					...
					if (f == selected) {
						...
						if (enabled != null) {
							...
							// "Closing outer ring", Toast.LENGTH_SHORT).show();
							enabled = null;
							animateOuterIn = true;
